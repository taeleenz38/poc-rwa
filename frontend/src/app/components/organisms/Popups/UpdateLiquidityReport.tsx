...
const UpdateLiquidityReport: React.FC<UpdateLRProps> = ({
  isOpen,
  onClose,
}) => {
  const [dailyAssets, setDailyAssets] = useState<string>("");
  const [weeklyAssets, setWeeklyAssets] = useState<string>("");
  const [dailyAssetsPercentage, setDailyAssetsPercentage] =
    useState<string>("");
  ...
  const UpdateLR = async () => {
    ...
    const payload = {
      totalDailyLiquidAssets: parseFloat(dailyAssets),
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_FILE_API}/liquid-assets/submit`,
        requestOptions
      );
      const result = await response.text();
      console.log("Success:", result);
      alert("Liquidity report submitted successfully!");
      onCloseModal();
    } catch (error) {
      console.error("Error submitting liquidity report:", error);
      alert("Failed to submit liquidity report.");
    }
  };
