...
async function main() {
  ...
  // admin (guardian) - DEFAULT_ADMIN_ROLE - ALLOWLIST_ADMIN - addTerm - setCurrentTermIndex - setValidTermIndexes
  // setter (guardian) - ALLOWLIST_SETTER - setAccountStatus
  
  let tx = await allowlist.connect(guardian).addTerm("Test Term 1", { gasPrice, gasLimit });
  await tx.wait()

  tx = await allowlist.connect(guardian).setValidTermIndexes([0], { gasPrice, gasLimit });
  await tx.wait()
