...
async function main() {
  ...
  let tx = await abbyManager.connect(managerAdmin).setClaimableTimestamp(block + 100, [FIRST_DEPOSIT_ID], { gasPrice, gasLimit });
  await tx.wait();
  console.log("Claimbale time set for deposit");
}
