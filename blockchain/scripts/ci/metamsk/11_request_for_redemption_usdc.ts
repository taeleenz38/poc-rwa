...
async function main() {
  ...
  const hfy = await ethers.getContract("HYF");
  const afy = await ethers.getContract("AYF");
  ...
  let tx = await afy.connect(user).approve(abbyManager.address, parseUnits("100", 18), { gasPrice, gasLimit });
  await tx.wait();
  console.log("Price set for deposit");

  const allowance = await afy.allowance(user.address, abbyManager.address);
  console.log(`Allowance of abbyManager for user: ${allowance.toString()} tokens`);
