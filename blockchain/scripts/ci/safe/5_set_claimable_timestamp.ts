...
async function main() {
  ...
  try {
    // Encode the setClaimableTimestamp function data
    const setClaimableTimestampData = abbyManager.interface.encodeFunctionData(
      "setClaimableTimestamp",
      [block + 100, [FIRST_DEPOSIT_ID]]
    );

    const txData: SafeTransactionDataPartial = {
      to: abbyManager.address,
      value: "0",
      data: setClaimableTimestampData,
    };
    ...
    console.log("--------------------setClaimableTimestamp transaction done!----------------------------------", receipt);
  } catch (error) {
