...
async function main() {
  ...
  const tx3 = await usdyManager.connect(managerAdmin).claimMint([FIRST_DEPOSIT_ID], { gasPrice, gasLimit });
  await tx3.wait(); 
  ...
  const tx6 = await usdy.connect(user).approve(usdyManager.address, parseUnits("20000", 18), { gasPrice, gasLimit });
  await tx6.wait(); 
  console.log("approving to spend usdy tokens!");
  

  //requesting redemptions
  const tx7 = await usdyManager.connect(user).requestRedemption(parseUnits("1000", 18), { gasPrice, gasLimit });
  await tx7.wait(); 
  console.log('redemption requested successfully!!');
  const HASH_OF_BANK = ethers.utils.hexZeroPad(ethers.utils.hexlify(69), 32);

  const tx8 = await usdyManager
    .connect(user)
  ...
  const tx10 = await usdc.connect(usdcWhaleSigner).transfer(assetSender.address, parseUnits("20000", 18), { gasPrice, gasLimit });
  await tx10.wait();

  console.log("transferred A$DC!");
  const tx11 = await usdc.connect(assetSender).approve(usdyManager.address, parseUnits("20000", 18), { gasPrice, gasLimit });
  await tx11.wait();
  const tx12 = await usdc.connect(assetSender).approve(user.address, parseUnits("20000", 18), { gasPrice, gasLimit });
  await tx12.wait();
  console.log("Approval provided for A$DC!");

  console.log('starting claim redemption!!');
  const tx13 = await usdyManager.connect(user).claimRedemption([FIRST_DEPOSIT_ID], { gasPrice, gasLimit });
  await tx13.wait();
